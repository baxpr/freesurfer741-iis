...
#    17   L hippocampus
#    53   R hippocampus
RL=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis x --imgval 17 )
AL=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis y --imgval 17 )
SL=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis z --imgval 17 )

RR=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis x --imgval 53 )
AR=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis y --imgval 53 )
SR=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis z --imgval 53 )


# View selected slices on T1, with surfaces
freeview \
    -v "${mri_dir}"/nu.mgz \
    -v "${mri_dir}"/lh.hippoAmygLabels.mgz:visible=1:colormap=lut \
...
freeview \
    -v "${mri_dir}"/nu.mgz \
    -v "${mri_dir}"/rh.hippoAmygLabels.mgz:visible=1:colormap=lut \
    -viewsize 400 400 --layout 1 --zoom 2.5 --viewport sag \
    -ras ${RR} ${AR} ${SR} \
...
# https://surfer.nmr.mgh.harvard.edu/fswiki/CoordinateSystems
#    16   brainstem
R=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis x --imgval 16 )
A=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis y --imgval 16 )
S=$(get_com.py --roi_niigz "${mri_dir}"/aseg.mgz --axis z --imgval 16 )

# View selected slices on T1, with surfaces
freeview \
    -v "${mri_dir}"/nu.mgz \
    -v "${mri_dir}"/brainstemSsLabels.mgz:visible=1:colormap=lut \
...
convert \
    ...
    -gravity NorthEast -pointsize 24 -annotate +20+50 "hippoAmygLabels.mgz (top)" \
    -gravity NorthEast -pointsize 24 -annotate +20+90 "brainstemSsLabels.mgz (bottom)" \
